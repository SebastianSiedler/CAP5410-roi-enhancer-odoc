"""
Training script for ResNet-based UNet segmentation.

This script trains a ResNet34-based UNet on glaucoma fundus images.
Optimized for GTX 2080 Ti with 11GB VRAM.
"""
...
def train_resnet_unet(
    ...
    use_clahe=False,
    model_type='resnet34',  # 'resnet34' or 'resnet18'
    pretrained=True
):
    """
    Main training function for ResNet-based UNet.
    ...
    Args:
        ...
        model_type: 'resnet34' or 'resnet18' (lighter)
    ...
    if pretrained:
        print("Using pretrained ImageNet weights for encoder")
    ...
    if model_type == 'resnet18':
        print("Using ResNet18-based UNet (lightweight)")
        model = ResNetUNetLite(n_channels=3, n_classes=3,
                               pretrained=pretrained)
    else:
        print("Using ResNet34-based UNet")
        model = ResNetUNet(n_channels=3, n_classes=3, pretrained=pretrained)
    ...
    class_weights = torch.tensor([
        1.0,  # Background
        1.0,  # Disc
        2.0   # Cup (harder to segment -> higher weight)
    ], device=device)
    ...
    # Lower LR for pretrained encoder, higher for decoder
    if pretrained:
        encoder_params = []
        decoder_params = []

        for name, param in model.named_parameters():
            if 'encoder' in name or 'conv1' in name or 'bn1' in name or 'layer' in name:
    ...
    for epoch in range(num_epochs):
        ...
        current_lr = optimizer.param_groups[-1]['lr']  # Get decoder LR
        ...
        # Also save best model based on disc IoU (most important metric)
    ...
    history_path = save_dir / 'training_history_resnet.json'
...
def plot_training_history(history, save_path=None):
    ...
    axes[0, 0].set_title('Training and Validation Loss (ResNet UNet)')
    axes[0, 0].legend()
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Train ResNet-based UNet for glaucoma segmentation')
    parser.add_argument('--root_dir', type=str, default='/home/robolab/dev/CAP5410-roi-enhancer-odoc',
    ...
    parser.add_argument('--save_dir', type=str, default='checkpoints_resnet',
    ...
    # Train model
    model, history = train_resnet_unet(
    ...
    save_path = Path(args.save_dir) / 'training_history_resnet.png'
