"""
EE-TransUNet: Edge-Enhanced TransUNet for Optic Disc/Cup Segmentation
Reference: https://github.com/wangyunyuwyy/EE-TransUNet
"""
# coding=utf-8
...
class DOConv2d(Module):
    ...
    Note that the groups parameter switches between DO-Conv (groups=1),
...
class DConv2dReLU(nn.Sequential):
    """DOConv2d + BatchNorm + ReLU block"""
    
    def __init__(self, in_channels, out_channels, kernel_size, padding=0, dilation=1, stride=1, use_batchnorm=True):
        conv = DOConv2d(
            in_channels,
            out_channels,
            kernel_size,
...
class DWConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, padding, stride=1):
        ...
        self.point_conv = nn.Conv2d(in_channels=in_channels, out_channels=out_channels, 
...
class CCF(nn.Module):
    """Cascaded Convolutional Fusion (CCF) block - Simplified version"""
    
    def __init__(self, in_channels, out_channels, padding=0, stride=1):
        super(CCF, self).__init__()
        # Use standard convolutions instead of DWConv to avoid channel mismatch issues
...
class DecoderBlock(nn.Module):
    """Simplified Decoder block for stable training"""
...
class DecoderCup(nn.Module):
    """Decoder with multiple decoder blocks"""
    ...
    def __init__(self, config):
        ...
        if self.config.n_skip != 0:
            ...
            # Ensure skip_channels has 4 elements
            while len(skip_channels) < 4:
...
class VisionTransformer(nn.Module):
    """Main EE-TransUNet model combining Transformer and Decoder"""
...
CONFIGS = {
    'ViT-Tiny': configs.get_tiny_config(),
