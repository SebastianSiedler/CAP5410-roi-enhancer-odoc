...
if __name__ == '__main__':
    # Test the models (use CPU to avoid memory issues in testing)
    device = torch.device('cpu')

    print("Testing ASPP-UNet Models...")
    print("=" * 70)

    # Test standard UNet for comparison
    from unet import UNet
    unet_standard = UNet(n_channels=3, n_classes=3,
    ...
    # Test Lightweight ASPP-UNet
    aspp_unet_lite = LightweightASPPUNet(
        n_channels=3,
        n_classes=3,
        base_channels=32,
    ...
    print(f"   Parameters: {count_parameters(unet_standard):,}")
    with torch.no_grad():
        out_std = unet_standard(x)
    print(f"   Input shape:  {x.shape}")
    print(f"   Output shape: {out_std.shape}")

    print("\n2. ASPP-UNet (full):")
    print(f"   Parameters: {count_parameters(aspp_unet):,}")
    param_increase = (count_parameters(
        aspp_unet) - count_parameters(unet_standard)) / count_parameters(unet_standard) * 100
    print(f"   Parameter increase: +{param_increase:.1f}%")
    with torch.no_grad():
        out_aspp = aspp_unet(x)
    print(f"   Input shape:  {x.shape}")
    print(f"   Output shape: {out_aspp.shape}")

    print("\n3. Lightweight ASPP-UNet:")
    print(f"   Parameters: {count_parameters(aspp_unet_lite):,}")
    with torch.no_grad():
        out_lite = aspp_unet_lite(x)
    print(f"   Input shape:  {x.shape}")
    ...
    print("\nKey insights:")
    print("- ASPP adds minimal parameters but captures multi-scale context")
    print("- Perfect for optic cup segmentation (needs both fine & broad context)")
    print("- Lightweight version good for faster experimentation")
