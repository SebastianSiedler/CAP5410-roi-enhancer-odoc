...
class GlaucomaDataset(Dataset):
    ...
    def _load_g1020(self) -> List[dict]:
        ...
        if not images_dir.exists():
            print(f"Warning: G1020 Images_Cropped/img not found at {images_dir}")
            return samples
    ...
    def _load_origa(self) -> List[dict]:
        ...
        # ORIGA images are directly in Images_Cropped, not in img subdirectory
        images_dir = origa_dir / 'Images_Cropped'
        masks_dir = origa_dir / 'Masks_Cropped'
        
        if not images_dir.exists():
            print(f"Warning: ORIGA Images_Cropped not found at {images_dir}")
            return samples
        
        # Scan images directly in Images_Cropped
        for image_path in images_dir.glob('*.jpg'):
            image_name = image_path.name
    ...
    def _filter_incomplete_masks(self, samples: List[dict]) -> List[dict]:
        ...
        if removed_count > 0:
            print(f"  Filtered out {removed_count} images with incomplete masks")
